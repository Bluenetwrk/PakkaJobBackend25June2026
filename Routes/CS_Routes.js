...
router.put("/uploadImage/:id", upload.single('image'), async (req, res) => {
    ...
    try {
        let result = await CSProfileModel.updateOne(
            { _id: req.params.id },
            ...
            { $set: { image: `https://nontech-backend.onrender.com/Images/${imagePath}` } }
...
router.put("/deleteImage/:id", async (req, res) => {
    ...
    const trimImagepath = comingImagepath.replace("https://nontech-backend.onrender.com/Images/", "")
...
router.post("/verifyOtp", async (req, res) => {
    ...
    try {
        ...
        let user = await CSProfileModel.findOne({ phoneNumber: PhoneNumber })
        if (user == null) {
            let saveUser = await CSProfileModel({ phoneNumber: PhoneNumber, isApproved: isApproved, ipAddress: ipAddress })
...
router.post("/Glogin", async (req, res) => {
    try {
        ...
        let user = await CSProfileModel.findOne({ email: email });
        if (user == null) {
            ...
            res.send({
                status: "success", token: gtoken, id: result._id, action: "registered"
            })
        } else {
            let Nowtime = Date()
            let result = await CSProfileModel.updateOne(
...
router.post("/NewCSRegistration", async (req, res) => {
    // console.log(req.body)

    try {
        let User = await new NewCSProfileRegistrationModel(req.body)
...
router.get("/getAllEmployees", verifyToken, async (req, res) => {
    try {
        let result = await CSProfileModel.find()
        res.send(result)
    } catch (err) {
...
router.delete("/deleteEmployee/:id", async (req, res) => {
    try {
        let result = await CSProfileModel.deleteOne({ _id: req.params.id })
        if (result) {
            res.send("success")
        }
    } catch (err) {
...
router.get("/getApprovedEmp", verifyToken, async (req, res) => {
    try {
        let result = await CSProfileModel.aggregate([{ $match: { isApproved: true } }])

        // let result = await CSProfileModel.find({"isApproved": { $exists: 1}}) // finds if the isapproved field exist or not(if exist send 1,if not send 0), not caring about wheather tue or false
...
router.get("/getNotApprovedEmp", verifyToken, async (req, res) => {
    try {
        let result = await CSProfileModel.aggregate([{ $match: { isApproved: false } }])
        if (result) {
            res.send(result)
        }
    } catch (err) {
...
router.get("/getFirmOrganisation", verifyToken, async (req, res) => {
    try {
        let result = await CSProfileModel.aggregate([{ $match: { TypeofOrganisation: "Firm" } }])
        if (result) {
            res.send(result)
        }
    } catch (err) {
...
router.get("/getPvt.Ltd.Organisation", verifyToken, async (req, res) => {
    try {
        let result = await CSProfileModel.aggregate([{ $match: { TypeofOrganisation: "Pvt.Ltd." } }])
        if (result) {
            res.send(result)
        }
    } catch (err) {
...
router.get("/getConsultancyOrganisation", verifyToken, async (req, res) => {
    try {
        let result = await CSProfileModel.aggregate([{ $match: { TypeofOrganisation: "Consultancy" } }])
        if (result) {
            res.send(result)
        }
    } catch (err) {
...
router.get("/getTodaysEmpProfile", verifyToken, async (req, res) => {
    try {
        let result = await CSProfileModel.find({ createdAt: { $gte: startDay, $lte: endDay } })
        if (result) {
            res.send(result)
            // console.log(result)
        }
...
router.get("/getAllemail", async (req, res) => {
    try {
        let result = await CSProfileModel.find({}, { email: 1, _id: 0 })
        res.send(result)
    } catch (err) {
...
router.get("/RecentLogin", async (req, res) => {
    try {
        let result = await CSProfileModel.find({ LogedInTime: { $gte: a, $lte: today } })
        if (result) {
            res.send(result)
        }
    } catch (err) {
...
router.get("/checkOnline", verifyToken, async (req, res) => {
    try {
        ...
        let result = await CSProfileModel.aggregate([{ $match: { online: true } }])
        if (result) {
            res.send(result)
        }
    } catch (err) {
...
router.get("/getTagsJobs/:name", async (req, res) => {
    ...
    try {
        let result = await CSProfileModel.aggregate([
...
router.get("/jobTagsIds/:id", async (req, res) => {
    ...
    try {
        ...
        const profile = await CSProfileModel.find({ _id: { $in: spliArray } })
...
router.get("/getTotalCount", async (req, res) => {
    try {
        let result = await CSProfileModel.estimatedDocumentCount()
...
router.delete("/ArchiveCheckBoxArray/:ids", verifyToken, async (req, res) => {
    ...
    try {
        ...
        let foundJobs = await CSProfileModel.find({ _id: { $in: comingIds } })
        ...
        if (foundJobs.length > 0) {
            ...
            let deletedJobs = await CSProfileModel.deleteMany({ _id: { $in: comingIds } })
        }
        res.send("success")
    } catch (err) {
        res.send("fail")
